...
def generate_annotation_report(records: list[dict]) -> dict:
    conflicting_images: dict[str, set[str]] = dict()
    reviewer_counts: dict[str, int] = dict()
    consensus_images = dict()
    ...
    for record in records:
        ...
        consensus_images.setdefault(image_id, list()).append(label)

    conflicting_images = {k: v for k, v in conflicting_images.items() if len(v) > 1}
    consensus_images = {k for k, v in consensus_images.items() if len(v) > 1 and len(set(v)) == 1}
